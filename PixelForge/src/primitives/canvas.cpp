...
void Canvas::fillRoundedRect(int x, int y, int w, int h, int radius, uint32_t color) {
    if (x >= fb_.width() || y >= fb_.height()) return;
    if (x + w < 0 || y + h < 0) return;
    ...
    fillRect(x1 + r, y1,       w - 2*r, r,        color);
    fillRect(x1,     y1 + r,   w,       h - 2*r,  color);
    fillRect(x1 + r, y2 - r,   w - 2*r, r,        color);
    ...
    for (int dy = 0; dy < r; ++dy) {
        for (int dx = 0; dx < r; ++dx) {
            if (dx*dx + dy*dy <= r*r) {
                int px = dx;
                int py = dy;
                drawPixel(x1 + r - px - 1, y1 + r - py - 1, color);
                drawPixel(x2 - r + px,     y1 + r - py - 1, color);
                drawPixel(x1 + r - px - 1, y2 - r + py,     color);
                drawPixel(x2 - r + px,     y2 - r + py,     color);
            }
        }
    }
}
void Canvas::fillGradientRect(int x, int y, int w, int h, uint32_t topColor, uint32_t bottomColor) {
    if (x >= fb_.width() || y >= fb_.height()) return;
    if (x + w < 0 || y + h < 0) return;
    ...
    uint8_t r1 = (topColor >> 16) & 0xFF;
    uint8_t g1 = (topColor >> 8)  & 0xFF;
    uint8_t b1 = topColor & 0xFF;

    uint8_t r2 = (bottomColor >> 16) & 0xFF;
    uint8_t g2 = (bottomColor >> 8)  & 0xFF;
    uint8_t b2 = bottomColor & 0xFF;

    switch (fb_.bpp()) {
        case 16: {
            for (int row = y1; row < y2; ++row) {
                float t = float(row - y1) / (height - 1);
                uint8_t r = static_cast<uint8_t>(r1 + (r2 - r1) * t);
                uint8_t g = static_cast<uint8_t>(g1 + (g2 - g1) * t);
                uint8_t b = static_cast<uint8_t>(b1 + (b2 - b1) * t);
                uint16_t color = Color::toRGB565((r << 16) | (g << 8) | b);
